/**
 * Script para criar as tabelas de Empresa no banco de dados
 * Execute: node scripts/create-company-tables.js
 */
...
async function main() {
  console.log('Criando tabelas de Empresa...');

  try {
    // Verificar se a tabela Company já existe
    ...
    if (tables && tables.length > 0) {
      ...
      `;
      console.log('✓ Tabela "company" criada');
    }

    // Verificar se a tabela CompanyRevenue já existe
    const revenueTables = await prisma.$queryRaw`
      SELECT TABLE_NAME 
      FROM information_schema.TABLES 
      WHERE TABLE_SCHEMA = DATABASE() 
      AND TABLE_NAME = 'companyrevenue'
    `;

    if (revenueTables && revenueTables.length > 0) {
      console.log('✓ Tabela "companyrevenue" já existe');
    } else {
      console.log('Criando tabela "companyrevenue"...');
      await prisma.$executeRaw`
      ...
      `;
      console.log('✓ Tabela "companyrevenue" criada');
    }

    // Verificar se a tabela CompanyProduct já existe
    const productTables = await prisma.$queryRaw`
      SELECT TABLE_NAME 
      FROM information_schema.TABLES 
      WHERE TABLE_SCHEMA = DATABASE() 
      AND TABLE_NAME = 'companyproduct'
    `;

    if (productTables && productTables.length > 0) {
      console.log('✓ Tabela "companyproduct" já existe');
    } else {
      console.log('Criando tabela "companyproduct"...');
      await prisma.$executeRaw`
      ...
      `;
      console.log('✓ Tabela "companyproduct" criada');
    }

    // Verificar se as colunas entityType e entityId existem na tabela fixedcost
